...
public class CanonUtils {
	...
	public static File download( __EdsObject directoryItem, File destination, boolean deleteAfterDownload ){
		...
		if (err == EdSdkLibrary.EDS_ERR_OK) {
			if( destination == null ){
				// chris
				Map<String, Object> camTmp = TempUtil.loadFotoTimbangCamera();
				
				String strFolder = (String) camTmp.get("folder");
				String strAWB = (String) camTmp.get("awb");
				String strFileName = (String) camTmp.get("filename");
				
				destination = new File(strFolder,strFileName);
